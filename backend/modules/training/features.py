...
def create_time_based_labels(
    ...
    Checks whether the price changes by at least ``min_percent_change`` %
    within ``future_minutes`` rows, per coin (grouped by mint).
    """
...
def add_pump_detection_features(
    ...
    if 'dev_sold_amount' in df.columns:
        ...
        for w in window_sizes:
            df[f'dev_sold_spike_{w}'] = (
                df['dev_sold_amount'] > df['dev_sold_amount'].rolling(w).mean() * 2
            ).astype(int).fillna(0)
            if include_flags:
    ...
    if 'whale_buy_volume_sol' in df.columns and 'whale_sell_volume_sol' in df.columns:
        ...
        for w in window_sizes:
            df[f'whale_activity_{w}'] = (
                df['whale_buy_volume_sol'].rolling(w).sum() +
                df['whale_sell_volume_sol'].rolling(w).sum()
            ).fillna(0)
            if include_flags:
    ...
    if 'unique_signer_ratio' in df.columns:
        for w in window_sizes:
            df[f'wash_trading_flag_{w}'] = (
                df['unique_signer_ratio'].rolling(w).mean() < 0.3
            ).astype(int).fillna(0)
            if include_flags:
    ...
    if 'net_volume_sol' in df.columns:
        for w in window_sizes:
            ...
            df[f'volume_flip_{w}'] = (
                np.sign(df['net_volume_sol']) != np.sign(df['net_volume_sol'].shift(w))
            ).astype(int).fillna(0)
            if include_flags:
    ...
    if 'price_close' in df.columns:
        for w in window_sizes:
            ...
            df[f'price_roc_{w}'] = (
                (df['price_close'] - df['price_close'].shift(w)) /
                df['price_close'].shift(w) * 100
            ).fillna(0)
            if include_flags:
